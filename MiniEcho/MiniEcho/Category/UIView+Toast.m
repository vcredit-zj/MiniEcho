...
static const NSString * CSToastImageViewKey     = @"CSToastImageViewKey";
...
- (void)makeToastInCenter:(NSString *)message{
...
#pragma mark - Toast Images Methods

- (void)makeToastLoadingImages
{
    // sanity
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSToastActivityViewKey);
    if (existingActivityView != nil) return;
    ...
    for (int i = 1 ; i <= 20; i++) {
        NSString *imageName = [NSString stringWithFormat:@"loading_%02d",i];
        UIImage *image = [UIImage imageNamed:imageName];
        [images addObject:image];
    }
    
    NSAssert(images.count > 0, @"loading图片不能为空");
    
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, CSToastActivityWidth, CSToastActivityHeight)];
    imageView.center = CGPointMake(activityView.bounds.size.width / 2, activityView.bounds.size.height / 2);
    imageView.animationImages = images;
    imageView.animationDuration = 1.2;
    [activityView addSubview:imageView];
    [imageView startAnimating];
    
    // associate the imageView view with self
    objc_setAssociatedObject (self, &CSToastImageViewKey, activityView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    
    [self addSubview:activityView];
    
    // show animated iamges
    [UIView animateWithDuration:CSToastFadeDuration
                          delay:0.0
                        options:UIViewAnimationOptionCurveEaseOut
                     animations:^{
...
- (void)hideToastLoadingImages
{
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSToastImageViewKey);
    if (existingActivityView != nil) {
        [UIView animateWithDuration:CSToastFadeDuration
                              delay:0.0
                            options:(UIViewAnimationOptionCurveEaseIn | UIViewAnimationOptionBeginFromCurrentState)
                         animations:^{
                             ...
                             [existingActivityView removeFromSuperview];
                             objc_setAssociatedObject (self, &CSToastImageViewKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                         }];
    }
}
